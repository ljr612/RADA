...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run hdock for multiple antibody files against a given antigen.")
    ...
    parser.add_argument("-o", "--output_dir", type=str, help="Output directory for hdock results.")
...
def run_hdock(args):
    ...
    os.system(f"../hdock/hdock {antigen} {input_file} -out {output_file}")
...
def main(args):
    ...
    with Pool() as pool:
        pool.map(run_hdock, [(file, antigen, antibody_dir, output_dir) for file in antibody_files])
